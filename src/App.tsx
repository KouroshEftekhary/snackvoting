...
const App: FC = () => {
    ...
    const onAddItem = (snack: Item ) => {
        const found = selectedItems.findIndex((element: Item) => element.id === snack.id);
        if (found === -1 && selectedItems.length < 3) {
            axios.post(`http://localhost:3000/snacks/vote/${snack.id}`, null,{
                headers: {
                    Authorization: `Bearer 33b55673-57c7-413f-83ed-5b4ae8d18827`,
                },
            }).then(() => {
                setSelectedItems([...selectedItems, snack])
            }).catch(
                (err) => {
                    console.error(err)
                }
        ...
        if (found !== -1) {
            message.error('You already voted for this item!')
        }
        if (selectedItems.length >= 3) {
            message.error('You can not vote for more than three items!')
        }
    }
    return (
        <div>
            <div className="u-isHidden">
                ...
                <svg id="icon-locate" xmlns="http://www.w3.org/2000/svg" viewBox="-33 35 32 32">
                    <circle fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" cx="-17" cy="46"
                            r="4"/>
                    <path fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
            ...
            <div className="site">
                ...
                <div className="site-bd">
                    ...
                    <div className="site-bd-section view-snacks-section">
                        <h3>
                            Currently In Stock
                        </h3>
                        <p>
                            Here are the snacks that are available in Nerdy kitchen no.
                        </p>
                        <Row>
                            {
                                snacks.map((item : Item) => {
                                    return (
                                        <Col key={item.id}  xs={{ span: 24 }} lg={{ span: 8 }}>
                                            <div className='snack-container'>
                                                <div className='triangle-topright'>
                                                    <span>{item.votes}</span>
                                                </div>
                                                <img className='snack-image' src={item.image} alt="snack picture"/>
                                            </div>
                                            <div className='snack-dec'>
                                                <b>{item.product}</b>
                                                <br/>
                    ...
                    <div className="site-bd-section vote-section">
                        <h3>
                            Snack voting
                        </h3>
                        <p>
                            Vote on the snacks you want to see on this month's rotation
                        </p>
                        <hr/>
                        <p>
                            {3 - selectedItems.length} votes remaining
                        </p>
                        <Row>
                            <Col className='text-left'  xs={{ span: 24 }} lg={{ span: 13, offset: 1 }}>
                                        ...
                                        snacks.map((item : Item) => {
                                            return (
                                                <div className='available-items'>
                                                    <span className='plus-button' onClick={() => onAddItem(item)}><PlusOutlined /></span>
                                                    <b>{item.product}</b>
                                                    <span className='number-badge'>{item.votes}</span>
                                                </div>
                                            )
                                        })
                                    }
                            ...
                            <Col className='text-left selection'  xs={{ span: 24 }} lg={{ span: 8 }}>
                                <div className='selection-header'>
                                    <h4>Selection</h4>
                                    <span className='number-badge'>{selectedItems.length}</span>
                                </div>
                                {
                                    selectedItems.map((item : Item) => {
                                        return (
                                            <div key={item.id} className='selected-items'>
                                                <b>{item.product}</b>
                                                <span className='number-badge'>{item.votes}</span>
                                            </div>
                                        )
                                        ...
                                        <div className="grid-col grid-col_2of3">
                                            <form className="inputForm">
                                                <div className="inputForm-item inputForm-item_half">
                                                    <label className="u-isVisuallyHidden" htmlFor="form-firstname">Fist
                                                        Name</label>
                                                    <input className="inputForm-text" type="text" name=""
                                                           placeholder="Fist Name"/>
                                                </div>
                                                <div className="inputForm-item inputForm-item_half">
                                                    <label className="u-isVisuallyHidden" htmlFor="form-lastname">Last
                                                        Name</label>
                                                    <input className="inputForm-text" type="text" name=""
                                                ...
                                                <div className="inputForm-item inputForm-item_half">
                                                    <label className="u-isVisuallyHidden" htmlFor="form-phonenumber">Phone
                                                        Number</label>
                                                    <input className="inputForm-text" type="text" name=""
                                                           placeholder="Phone Number"/>
                                                </div>
                                                <div className="inputForm-item">
                                                    <label className="u-isVisuallyHidden"
                                                           htmlFor="form-message">Message</label> <textarea
                                                    className="inputForm-area" placeholder="Message"></textarea>
                                                </div>
                                                <div className="inputForm-item">
                                                    <div className="inputForm-item-checkGroup">
                                                        <input id="form-employee" className="u-isVisuallyHidden"
                                                               type="checkbox"/>
                                                        <label htmlFor="form-employee">I am a Nerdery employee</label>
                                                    </div>
                                                    <div className="inputForm-item-checkGroup">
                                                        <input id="form-notify" className="u-isVisuallyHidden"
                                                               type="checkbox"/>
